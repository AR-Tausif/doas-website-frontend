...
export default function MainSidebar() {
  ...
  const currentSubcategoryId = pathParts[4] ? Number.parseInt(pathParts[4]) : null
  ...
  return (
    <div className="fixed left-[70px] top-0 h-full w-[250px] bg-[#f5f9f6] border-r overflow-y-auto">
      ...
      <div className="space-y-1">
        {categories
          .filter((category) => searchQuery === "" || category.name.toLowerCase().includes(searchQuery.toLowerCase()))
          .map((category) => (
            <div key={category.id} className="mb-2">
              <div className="px-4 py-2">
                <Link href={`/category/${category.id}/subcategory/${category.subcategories[0]?.id || 1}`}>
                  <div
                    className={`flex items-center gap-3 p-2 rounded-md ${
                      currentCategoryId === category.id ? "bg-[#e8f3ee]" : "hover:bg-gray-100"
                    }`}
                  >
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500">
                        {category.subcategories.length} Subcategories | {category.duaCount} Duas
                      </p>
                    </div>
                  </div>
                </Link>
                  ...
                  <ul className="space-y-1">
                    {category.subcategories.map((subcategory) => (
                      <li key={subcategory.id}>
                        <Link href={`/category/${category.id}/subcategory/${subcategory.id}`}>
                          <div
